 ...
 *
 */
package org.openjdk.jmc.console.ui.shenandoahvisualizer;
...
import static org.openjdk.jmc.console.ui.shenandoahvisualizer.RegionState.*;
...
public class ShenandoahVisualizer implements IConsolePageStateHandler {
	...
	protected void createPageContent(IManagedForm managedForm, IConnectionHandle connection) throws Exception {
		...
		setPid(connection.getServerDescriptor().getJvmInfo().getPid());
				...
				if (bs != null) {
					for (int t = 0; t < snapshot.regionCount(); t++) {
						if (bs.get(t)) {
							...
							int f_recty = (int) (((double) f / (double) cols + 0.5) * sqSize);
							int t_rectx = (int) ((t % cols + 0.5) * sqSize);
							int t_recty = (int) (((double) t / (double) cols + 0.5) * sqSize);
	...
	public void createVisualizer(Composite parent, Rectangle size, ScrolledForm form) throws Exception {
		...
		setImage(new Image(Display.getDefault(), 1000, 1000));
		setGC(new GC(image));
		fillLegend();
		...
		regionsPanel.addPaintListener(new PaintListener() {
			public void paintControl(PaintEvent e) {
				...
				e.gc.drawImage(image, 0, 0, image.getBounds().width, image.getBounds().height, 0, 0,
						regionsPanel.getBounds().width, regionsPanel.getBounds().height);
			}
		});
	...
	private synchronized static void setPid(int pid) {
		ShenandoahVisualizer.pid = pid;
	}

	private synchronized static void setRender(Render render) {
		ShenandoahVisualizer.render = render;
	}

	private synchronized static void setGC(GC gc) {
		ShenandoahVisualizer.g = gc;
	}

	private synchronized static void setImage(Image image) {
		ShenandoahVisualizer.image = image;
	}
